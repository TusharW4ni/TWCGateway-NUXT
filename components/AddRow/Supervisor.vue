...
import { validateNewSupervisor } from "./validator.ts";
...
const fetchedTasks = await useFetch("/api/tasks/get", {
  method: "GET",
  onRequestError({ error }) {
    console.log({ error });
    notify("Error: Cannot fetch tasks", "error");
  },
  onResponse({ response }) {
...
async function handleOnEmSubmit() {
  ...
  const errors = await validateNewSupervisor(trimmedRow);
  console.log({ errors });
  if (errors.length > 0) {
  ...
  try {
    const res = await $fetch("/api/users/post/supervisor", {
      method: "POST",
      body: {
  ...
  <table class="w-full">
    <tbody>
      <tr class="">
        ...
        <td class="md:px-2 text-center">
          <Select
            v-if="fetchedTasks.tasks?.length > 0"
            :type="'Tasks'"
            :options="fetchedTasks.tasks"
